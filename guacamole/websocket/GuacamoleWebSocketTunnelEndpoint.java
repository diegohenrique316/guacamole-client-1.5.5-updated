...
public abstract class GuacamoleWebSocketTunnelEndpoint extends Endpoint {
	...
	private int udpPort = 9876; // Example UDP port
	private volatile boolean running = true; // Flag to control the UDP receiver
    ...
    public void onOpen(final Session session, EndpointConfig config) {
        ...
        Thread readThread = new Thread() {
            ...
            public void run() {
				...
				//System.out.println("readThread is running, waiting for messages...");
		...
		/*try {
            ...
            new Thread(this::receiveUdpPackets).start();
        } catch (SocketException e) {
	        logger.error("Failed to create UDP socket: {}", e.getMessage());
	        closeConnection(session, GuacamoleStatus.SERVER_ERROR);
	        return; // Exit the method after handling the exception
...
//System.out.println("onMessage: " + instruction.toString());
    ...
    public void onClose(Session session, CloseReason closeReason) {
		running = false; // Stop the UDP receiver
        try {
            if (tunnel != null)
                tunnel.close();
        }
    ...
    // Start the UDP receiver in a new thread
    private void receiveUdpPackets() {
        byte[] buffer = new byte[1024];
        System.out.println("receiveUdpPackets...............");
        while (running) {
        	System.out.println("receiveUdpPackets");
            try {
                ...
                System.out.println("receiveUdpPackets: " + message);
		        ...
		        GuacamoleWriter writer = new FilteredGuacamoleWriter(tunnel.acquireWriter(), new GuacamoleFilter() {
		            ...
		            public GuacamoleInstruction filter(GuacamoleInstruction instruction)
		                ...
		                // Filter out all tunnel-internal instructions
		                if (instruction.getOpcode().equals(GuacamoleTunnel.INTERNAL_DATA_OPCODE)) {
		System.out.println("UDP onMessage: " + instruction.toString());
		                    // Respond to ping requests
		                    List<String> args = instruction.getArgs();
		                    if (args.size() >= 2 && args.get(0).equals(PING_OPCODE)) {
				...
				System.out.println("UDP write: " + message);
		        try {
		            // Write received message
		            writer.write(message.toCharArray());
		        }
